...
st.caption("Versión sin Plotly: costo mensual detallado, horario editable, bonos, recargos, dominicales, festivos y aportes empresa.")
...
def build_calendar(year, month, start_cycle_day, start_t, end_t, ordinary_day, holidays_worked_manual):
    ...
    for d in range(1, days_in_month + 1):
        ...
        if not works:
            label, color = "DESCANSO", "#D9D9D9"
        elif is_holiday:
            label, color = "FESTIVO", "#CFE2F3"
        elif is_sunday:
            label, color = "DOMINICAL", "#F4CCCC"
        elif night_hours > 0:
            label, color = "LABORA / NOCT.", "#D9D2E9"
        else:
            label, color = "LABORA", "#D9EAD3"
        records.append({"Fecha": current, "Dia": d, "Semana_mes": ((d - 1) // 7) + 1,
        "Dia_semana": ["Lun", "Mar", "Mie", "Jue", "Vie", "Sab", "Dom"][current.weekday()],
        "Dia_ciclo_14x7": day_cycle, "Estado": "LABORA" if works else "DESCANSO",
        "Etiqueta": label, "Color": color, "Es_domingo": is_sunday, "Es_festivo": is_holiday,
        "Horas_totales": worked_hours, "Horas_diurnas": day_hours, "Horas_nocturnas": night_hours,
        "Horas_ordinarias_diurnas": ordinary_daytime, "Horas_ordinarias_nocturnas": ordinary_night,
        "Horas_extra_diurnas": extra_day, "Horas_extra_nocturnas": extra_night})
        cycle_day += 1
...
breakdown = pd.DataFrame({
    "Bloque": ["A. COSTO FIJO","A. COSTO FIJO","A. COSTO FIJO","A. COSTO FIJO","A. COSTO FIJO","B. BONO VARIABLE","C. HORAS Y RECARGOS","C. HORAS Y RECARGOS","C. HORAS Y RECARGOS","C. HORAS Y RECARGOS","C. HORAS Y RECARGOS","C. HORAS Y RECARGOS","D. APORTES / PARAFISCALES","D. APORTES / PARAFISCALES","D. APORTES / PARAFISCALES","D. APORTES / PARAFISCALES","D. APORTES / PARAFISCALES","D. APORTES / PARAFISCALES","D. APORTES / PARAFISCALES","TOTAL"],
    "Concepto": ["1 SMLV / salario base","Bono disponibilidad","Bono resultados","Bono comunicación","Valor transporte","Bono por facturación","Horas diurnas ordinarias","Horas nocturnas ordinarias base","Recargo nocturno","Horas extras diurnas","Horas extras nocturnas","Dominicales y festivos","Salud empleador","Pensión empleador","ARL","Caja de compensación","SENA","ICBF","Prestaciones estimadas","TOTAL POR CONDUCTOR"],
    "Horas/Cantidad": ["","","","","",f"{porcentaje_bono*100:.1f}% sobre {cop(valor_facturado)}",f"{ordinary_day_hours:.1f} h",f"{ordinary_night_hours:.1f} h",f"{night_hours:.1f} h",f"{extra_day_hours:.1f} h",f"{extra_night_hours:.1f} h",f"{sundays_worked} dom. / {holidays_assigned} fest.",f"{salud_emp_rate*100:.2f}%",f"{pension_emp_rate*100:.2f}%",f"{arl_rate*100:.3f}%",f"{caja_rate*100:.2f}%",f"{sena_rate*100:.2f}%",f"{icbf_rate*100:.2f}%",f"{prestaciones_rate*100:.2f}%",""],
    "Valor": [salary,bono_disponibilidad,bono_resultados,bono_comunicacion,valor_transporte,variable_bonus,ordinary_day_cost,ordinary_night_base_cost,night_surcharge_cost,extra_day_cost,extra_night_cost,sunday_cost+holiday_cost,salud_emp,pension_emp,arl_emp,caja_emp,sena_emp,icbf_emp,prestaciones,monthly_cost_one]
})
...
chart_data = breakdown[breakdown["Concepto"] != "TOTAL POR CONDUCTOR"].groupby("Bloque", as_index=True)["Valor"].sum()
st.bar_chart(chart_data)

st.subheader("📈 Horas estimadas")
hours_df = pd.DataFrame({"Horas": [ordinary_day_hours, ordinary_night_hours, extra_day_hours, extra_night_hours, sunday_hours, holiday_hours]}, index=["Diurnas ordinarias","Nocturnas ordinarias","Extras diurnas","Extras nocturnas","Dominicales","Festivas"])
st.bar_chart(hours_df)
...
for week in sorted(df["Semana_mes"].unique()):
    ...
    for _, r in week_data.iterrows():
        ...
        html += f"<td style='background:{r['Color']}; padding:10px; border-radius:12px; text-align:center; border:1px solid #ECECEC;'>{cell}</td>"
    html += "</tr>"
html += "</table>"
...
for week, group in df.groupby("Semana_mes"):
    ...
    cost_week = ((h_dia * hour_value) + (h_noc * hour_value) + (h_noc * hour_value * night_rate) + (h_ext_dia * hour_value * (1 + extra_day_rate)) + (h_ext_noc * hour_value * (1 + extra_night_rate)) + (h_dom * hour_value * sunday_rate) + (h_fes * hour_value * holiday_rate))
    weekly_rows.append({"Semana": f"Semana {week}", "Horas totales": h_total, "Horas diurnas": h_dia, "Horas nocturnas": h_noc, "Extras diurnas": h_ext_dia, "Extras nocturnas": h_ext_noc, "Horas dominicales": h_dom, "Horas festivas": h_fes, "Costo horas/recargos aprox.": cost_week})
...
detail = df[["Fecha","Semana_mes","Dia_semana","Dia_ciclo_14x7","Estado","Etiqueta","Horas_totales","Horas_diurnas","Horas_nocturnas","Horas_ordinarias_diurnas","Horas_ordinarias_nocturnas","Horas_extra_diurnas","Horas_extra_nocturnas","Es_domingo","Es_festivo"]].copy()
